...
In this section, we will see how **physLP** can be installed in your Python projects.
...
Here, we want to create a new Python project with a dependency on **physLP**.
...
Then, we just need to add the dependencies to your project, typically dependency to **physLP**, with the command:

.. code-block:: shell

   uv add physlp

You can redo this command for all the dependencies you might need by replacing the "physLP" by the other dependency name (the name ususally used in the command ``pip``).
...
.. tab-set::
   ...
   .. tab-item:: Anaconda tools (Spyder Jupyter)
      ...
      To be able to use another virtual environment, *Spyder* and *Jupyter* connects to specific kernels, respectively *spyder-kernels* and *ipykernel*, which are included in **physLP** library dependencies.
...
To finish, some routines may be provided with the library **physLP** :doc:`(see Reference Guide) <routines>`.
To use them, you can just download `routines source codes <https://github.com/LancelotPincet/physLP/tree/main/src/physlp/routines>`_.
...
.. dropdown:: Using *Anaconda* distribution
   ...
   *Anaconda* is a Python distribution designed for scientific programming.
   ...
   Here we will explain how to setup up virtual environments dependent on **physLP** that you will be able to use on various projects.
   ...
   However some dependencies for this Python project might not be available in this *base* environment, like **physLP**.
   ...
   - create venv: ``conda create -n venv_physLP python``
   - activate venv: ``conda activate venv_physLP``
   - update pip: ``conda install pip``
   - add *Spyder* and *Jupyter* kernels: ``conda install spyder-kernels ipykernel``
   - install **physLP**: ``pip install physlp``
...
.. dropdown:: Manual project
   ...
   6. Install **physLP** from terminal: ``pip install physlp``
...
If ever you want to play with the library source code, to apply your own modifications, or even to contribute, you can access it on the `GitHub page <https://github.com/LancelotPincet/physLP>`_.
...
Now, you can clone in your current directory the `source code folder <https://github.com/LancelotPincet/physLP>`_ with the following command

.. code-block:: shell

   git clone https://github.com/LancelotPincet/physLP.git
