...
Modules go in `src/physlp/modules/`.
Routines go in `src/physlp/routines/`.
...
- Public objects must be exposed in `physlp/__init__.py`
...
```python
# RIGHT
from physlp import MyObject

# WRONG — never import from internal paths
from physlp.modules.myobject_LP.myobject import MyObject
```

Every public object must be registered in `physlp/__init__.py`:

```python
sources = {
    "<name>": "physlp.modules.<name>_LP.<name>",
}
```
